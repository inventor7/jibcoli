<template>
  <FormField v-slot="{ field }" name="language">
    <FormItem class="flex flex-col w-full border sm:border-none rounded-full">
      <Popover v-if="isTablet" v-model:open="isOpen">
        <PopoverTrigger as-child>
          <FormControl>
            ...
            >
              <span
                class="flex flex-row justify-center text-2xl items-center gap-2"
        ...
        <DrawerContent class="h-[90vh]">
          ...
          <div class="grid grid-cols-2 grid-rows-4 h-full gap-2 pt-4 p-2">
            ...
            <div
              ...
              class="flex flex-col justify-around items-center w-full h-full p-3 text-gray-600 border-[2px] rounded-lg"
              v-for="cat in categories"
            >
              <component :size="44" :stroke-width="1.3" :is="cat.icon" />
              <span class="select-none text-sm text-center w-full"
                >{{ $t(cat.name) }}
              </span>
            </div>
          </div>
